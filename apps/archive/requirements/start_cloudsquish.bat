...
if not exist "%SERVER_ROOT%\archives\static\uploads" mkdir "%SERVER_ROOT%\archives\static\uploads"

REM
set ENV_FILE=%SERVER_ROOT%\.env

REM
if not exist "%ENV_FILE%" (
    (
        echo SECRET_KEY=AAAAB3NzaC1yc2EAAAADAQABAAABAQCjrFncQTzkFeeK7GB7b2Y6CqGBAiuovcWeWr3W1dqwZHu86kJxuNSAEUnJ
        echo DEV_DATABASE_URL=sqlite:///%SERVER_ROOT%\database\app.db
        ...
        echo ARCHIVE_DEV_DATABASE_URL=sqlite:///archive.db
        echo ARCHIVE_TEST_DATABASE_URL=sqlite:///archive-test.db
        echo ARCHIVE_PRODUCTION_DATABASE_URL=sqlite:///archive-prod.db
        echo ARCHIVE_STATIC_DIR=static/uploads
    ) > "%ENV_FILE%"
)

REM
start %SERVER_ROOT%\sails.exe
exit
